...
const ViewUser = ({ open, onClose, user }) => {
  return (
    <Modal show={open} popup>
      <Modal.Header> <h3 className="text-xl font-medium text-secondary px-3 pb-3">
      ...
      <Modal.Body className="text-secondary">
        <div className="space-y-6">
         <div className="flex gap-4">
          <div>
            <img src={user?.avatar} alt="" srcset="" className="w-20 h-20 rounded-lg"/>
          </div>
          <div className="bg-primary-600 px-3 py-4 rounded-md text-pay shadow-md shadow-primary-600">
           <div>
           <h1 className="text-sm">User Balance: <strong>₦{Number(user?.account_balance).toLocaleString("en-US")}</strong></h1>
           </div>
           <hr className="mt-2"/>
          <div className="mt-1">
            <h1 className="text-sm">Account Number: <strong>{user?.account_number}</strong></h1>
          </div>
          </div>
          <div className="flex flex-col gap-3 shadow-md shadow-primary-600 px-3 py-3 rounded-md">
         <div><h1>BVN Number: {user?.bvn_number}</h1></div>

          <div>
          <h1>NIN Number: {user?.nin_number}</h1>
          </div>
         </div>
         </div>
